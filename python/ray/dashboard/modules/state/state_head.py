...
class StateHead(SubprocessModule, RateLimitedModule):
    ...
    @routes.get("/api/v0/network_topology")
    @RateLimitedModule.enforce_max_concurrent_calls
    async def network_topology(self, req: aiohttp.web.Request) -> aiohttp.web.Response:
        try:
            keys = ray.experimental.internal_kv._internal_kv_list(
                "edge_samples:", namespace="network_sense"
            )

            reports_by_src = {}
            for key in keys:
                data = ray.experimental.internal_kv._internal_kv_get(
                    key, namespace="network_sense"
                )
                if not data:
                    continue
                ...
                src = report.get("node_id")
                if not src:
                    continue

                prev = reports_by_src.get(src)
                if prev is None:
                    reports_by_src[src] = report
                    continue

                ts_ms = report.get("ts_ms")
                prev_ts_ms = prev.get("ts_ms")
                if isinstance(ts_ms, int) and isinstance(prev_ts_ms, int) and ts_ms > prev_ts_ms:
                    reports_by_src[src] = report
            ...
            for src, report in reports_by_src.items():
                nodes.add(src)
                report_ts_ms = report.get("ts_ms")

                edges_list = report.get("edges") or []
                if not isinstance(edges_list, list):
                    edges_list = []

                node_meta[src] = {
                    "node_id": src,
                    "last_report_ts_ms": report_ts_ms,
                    "num_out_edges": len(edges_list),
                }
                    ...
                    edges.append(
                        {
                            ...
                            "rtt_ms": edge.get("rtt_ms"),
                            "rtt_p50_ms": edge.get("rtt_p50_ms"),
                            "jitter_ms": edge.get("jitter_ms"),
                            "bw_mbps": edge.get("bw_mbps"),
                            "bw_p50_mbps": edge.get("bw_p50_mbps"),
                        }
                    )
            ...
            for node_id in nodes:
                node_meta.setdefault(
                    ...
                    {
                        "node_id": node_id,
                        "last_report_ts_ms": None,
                        "num_out_edges": 0,
                    },
                )

            nodes_out = sorted(node_meta.values(), key=lambda n: n.get("node_id") or "")
            edges_out = sorted(
                edges,
                key=lambda e: (e.get("src_node_id") or "", e.get("dst_node_id") or ""),
            )

            snapshot = {
                "generated_at_ms": int(datetime.utcnow().timestamp() * 1000),
                "directed": True,
                "num_reports": len(reports_by_src),
