...
def network_status(address: str):
    address = services.canonicalize_bootstrap_address_or_die(address)
    gcs_client = ray._raylet.GcsClient(address=address)
    _check_ray_version(gcs_client)
    ray.experimental.internal_kv._initialize_internal_kv(gcs_client)
    ...
    table = [["src_node_id", "dst_node_id", "rtt_ms", "rtt_p50_ms", "jitter_ms", "bw_mbps", "bw_p50_mbps"]]
    for key in keys:
        data = ray.experimental.internal_kv._internal_kv_get(key, namespace="network_sense")
        if not data:
            continue
        try:
        ...
        for edge in report.get("edges", []):
            table.append(
                [
                    ...
                    str(edge.get("rtt_ms")),
                    str(edge.get("rtt_p50_ms")),
                    str(edge.get("jitter_ms")),
                    str(edge.get("bw_mbps")),
                    str(edge.get("bw_p50_mbps")),
                ]
            )

    if len(table) == 1:
        print(
            "No network-sensing reports found. "
            "Enable RayConfig::network_sense_enabled and wait a few seconds."
        )
        return
