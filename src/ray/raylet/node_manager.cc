...
void NodeManager::RegisterGcs() {
  ...
  auto on_node_change_subscribe_done = [this](Status status) {
    ...
    if (RayConfig::instance().network_sense_enabled()) {
      periodical_runner_->RunFnPeriodically(
          [this] { NetworkSenseTick(); },
          RayConfig::instance().network_sense_tick_ms(),
          "NodeManager.NetworkSenseTick");
    }
  };
...
void NodeManager::NetworkSenseTick() {
  const auto now_ms = current_time_ms();
  const auto ttl_ms = RayConfig::instance().network_sense_rtt_ttl_ms();
  const auto alpha = RayConfig::instance().network_sense_rtt_alpha();
  const auto sample_window =
      std::max<uint32_t>(1, RayConfig::instance().network_sense_rtt_m());
  const auto concurrency_per_node =
      RayConfig::instance().network_sense_rtt_concurrency_per_node();
  const auto ping_timeout_ms = std::max<uint64_t>(
      1, std::min<uint64_t>(RayConfig::instance().network_sense_tick_ms() / 2, ttl_ms));

  // Fire pings.
  for (const auto &[node_id, address] : remote_node_manager_addresses_) {
    auto &in_flight = network_sense_in_flight_per_node_[node_id];
    if (in_flight >= static_cast<int>(concurrency_per_node)) {
      continue;
    }
    in_flight++;

    rpc::NetPingRequest req;
    req.set_src_node_id(self_node_id_.Binary());
    req.set_seq(++network_sense_seq_);
    ...
    const int64_t t0_ns = current_time_ns();

    raylet_client->NetPing(
        req,
        [this, node_id, t0_ns, alpha, sample_window](const Status &status,
                                                     rpc::NetPingReply &&) {
          auto it = network_sense_in_flight_per_node_.find(node_id);
          if (it != network_sense_in_flight_per_node_.end() && it->second > 0) {
            it->second--;
          }
          ...
          const int64_t t1_ns = current_time_ns();
          const double rtt_ms = static_cast<double>(t1_ns - t0_ns) / 1e6;

          auto &samples = network_sense_rtt_samples_ms_[node_id];
          samples.push_back(rtt_ms);
          while (samples.size() > static_cast<size_t>(sample_window)) {
            samples.pop_front();
          }

          std::vector<double> tmp(samples.begin(), samples.end());
          const size_t p50_index = tmp.size() / 2;
          std::nth_element(tmp.begin(), tmp.begin() + p50_index, tmp.end());
          const double p50_ms = tmp[p50_index];

          const size_t p95_index = (tmp.size() * 95 + 99) / 100 - 1;
          std::nth_element(tmp.begin(), tmp.begin() + p95_index, tmp.end());
          const double p95_ms = tmp[p95_index];

          auto &ewma = network_sense_rtt_ms_ewma_[node_id];
          if (ewma <= 0) {
            ewma = p50_ms;
          } else {
            ewma = alpha * p50_ms + (1.0 - alpha) * ewma;
          }

          network_sense_rtt_ms_p50_[node_id] = p50_ms;
          network_sense_jitter_ms_[node_id] = std::max(0.0, p95_ms - p50_ms);
          network_sense_last_update_ms_[node_id] = current_time_ms();
        },
        /*timeout_ms=*/static_cast<int64_t>(ping_timeout_ms));
  }



  // Fire bandwidth probes (upload throughput estimate via gRPC payload).
  if (RayConfig::instance().network_sense_bw_enabled()) {
    const auto bw_sample_window =
        std::max<uint32_t>(1, RayConfig::instance().network_sense_bw_m());
    const auto bw_alpha = RayConfig::instance().network_sense_bw_alpha();
    const auto bw_concurrency_per_node =
        RayConfig::instance().network_sense_bw_concurrency_per_node();
    const auto payload_bytes = RayConfig::instance().network_sense_bw_payload_bytes();

    std::string payload;
    payload.resize(payload_bytes, 'x');

    for (const auto &[node_id, address] : remote_node_manager_addresses_) {
      auto &in_flight = network_sense_bw_in_flight_per_node_[node_id];
      if (in_flight >= static_cast<int>(bw_concurrency_per_node)) {
        continue;
      }
      in_flight++;

      rpc::NetBandwidthProbeRequest req;
      req.set_src_node_id(self_node_id_.Binary());
      req.set_seq(++network_sense_seq_);
      ...
      raylet_client->NetBandwidthProbe(
          req,
          [this, node_id, t0_ns, bw_alpha, bw_sample_window, payload_bytes](
              const Status &status, rpc::NetBandwidthProbeReply &&) {
            auto it = network_sense_bw_in_flight_per_node_.find(node_id);
            if (it != network_sense_bw_in_flight_per_node_.end() && it->second > 0) {
              it->second--;
            }
            ...
            const double elapsed_s =
                std::max(1e-9, static_cast<double>(t1_ns - t0_ns) / 1e9);
            const double mbps = (static_cast<double>(payload_bytes) * 8.0) /
                                (elapsed_s * 1e6);

            auto &samples = network_sense_bw_samples_mbps_[node_id];
            samples.push_back(mbps);
            while (samples.size() > static_cast<size_t>(bw_sample_window)) {
              samples.pop_front();
            }
            ...
            const size_t p50_index = tmp.size() / 2;
            std::nth_element(tmp.begin(), tmp.begin() + p50_index, tmp.end());
            const double p50_mbps = tmp[p50_index];

            auto &ewma = network_sense_bw_mbps_ewma_[node_id];
            if (ewma <= 0) {
              ewma = p50_mbps;
            } else {
              ewma = bw_alpha * p50_mbps + (1.0 - bw_alpha) * ewma;
            }

            network_sense_bw_mbps_p50_[node_id] = p50_mbps;
          },
          /*timeout_ms=*/static_cast<int64_t>(
              std::max<uint64_t>(1000, RayConfig::instance().network_sense_tick_ms())));
    }
  }
  // Publish best-effort snapshot to Internal KV.
  std::string report = absl::StrCat("{\"node_id\":\"",
                                    self_node_id_.Hex(),
                                    "\",\"ts_ms\":",
                                    now_ms,
                                    ",\"edges\":[");
  bool first = true;
  for (const auto &[dst_node_id, ewma_rtt_ms] : network_sense_rtt_ms_ewma_) {
    const auto ts_it = network_sense_last_update_ms_.find(dst_node_id);
    if (ts_it == network_sense_last_update_ms_.end()) {
      continue;
    }
    if (now_ms - ts_it->second > ttl_ms) {
      continue;
    }
    if (!first) {
    ...
    double rtt_p50_ms = ewma_rtt_ms;
    if (auto it = network_sense_rtt_ms_p50_.find(dst_node_id);
        it != network_sense_rtt_ms_p50_.end()) {
      rtt_p50_ms = it->second;
    }
    double jitter_ms = 0.0;
    double bw_mbps = 0.0;
    if (auto it = network_sense_bw_mbps_ewma_.find(dst_node_id);
        it != network_sense_bw_mbps_ewma_.end()) {
      bw_mbps = it->second;
    }
    double bw_p50_mbps = 0.0;
    if (auto it = network_sense_bw_mbps_p50_.find(dst_node_id);
        it != network_sense_bw_mbps_p50_.end()) {
      bw_p50_mbps = it->second;
    }
    if (auto it = network_sense_jitter_ms_.find(dst_node_id);
        it != network_sense_jitter_ms_.end()) {
      jitter_ms = it->second;
    }
  ...
  const std::string key =
      absl::StrCat(kNetworkSensingNodeReportKeyPrefix, self_node_id_.Hex());
  gcs_client_.InternalKV().AsyncInternalKVPut(
      kNetworkSensingNamespace,
      key,
      report,
      /*overwrite=*/true,
      rpc::GetGcsTimeoutMs(),
      [](Status status, std::optional<bool> added) {});
}
...
void NodeManager::NodeRemoved(const NodeID &node_id) {
  ...
  network_sense_in_flight_per_node_.erase(node_id);
  network_sense_rtt_samples_ms_.erase(node_id);
  network_sense_rtt_ms_ewma_.erase(node_id);
  network_sense_rtt_ms_p50_.erase(node_id);
  network_sense_jitter_ms_.erase(node_id);
  network_sense_bw_in_flight_per_node_.erase(node_id);
  network_sense_bw_samples_mbps_.erase(node_id);
  network_sense_bw_mbps_ewma_.erase(node_id);
  network_sense_bw_mbps_p50_.erase(node_id);
  network_sense_last_update_ms_.erase(node_id);
...
void NodeManager::HandleNetPing(rpc::NetPingRequest request,
                               rpc::NetPingReply *reply,
                               rpc::SendReplyCallback send_reply_callback) {
...
void NodeManager::HandleNetBandwidthProbe(rpc::NetBandwidthProbeRequest request,
                                         rpc::NetBandwidthProbeReply *reply,
                                         rpc::SendReplyCallback send_reply_callback) {
  reply->set_seq(request.seq());
  reply->set_payload_size(static_cast<uint64_t>(request.payload().size()));
  send_reply_callback(Status::OK(), nullptr, nullptr);
}
